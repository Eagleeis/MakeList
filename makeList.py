...
# Default extensions which will be party of m3u lists
extensionsPictures		= ".jpg,.bmp,.gif,.png,.tif,.tiff"
...
class MakeList:
	...
	#######################################################################################################################
	def __init__( self, listsFolder, extensions, ignore, outputMode, fmtOnlyEntries, fmtAllSubEntries, fmtLists,
				  snippet, writeEmptyLists, outputType, dryMode, verbose, verboseVerbose ):
		...
		if outputType is None:
			...
			defListExtension			= ".lst"
			defFmtLists					= "{0}{3}"
		elif outputType == "m3u":							# Create m3u lists in all folders and sub folders
		...
		self.__outputEncoding		= defOutputEncoding
	...
	def __getOutput( self, lines ):
		if self.__outputType == "m3uExt":
			return "file:///" + "\nfile:///".join( lines )
		return "\n".join( lines )
#		if not self.__outputType or self.__outputType == "m3u" or self.__outputType == "fileList":
#			return "\n".join( lines )
...
#			raise Exception( "Unsupported output type \"{}\" found!".format( self.__outputType ) )
	...
	def __scanDir( self, prefix, d, scanSubDirectories, preventRedundantLists ):
		...
		files		= [ _ for _ in allItems if _ not in subDirs and ( os.path.splitext(_)[ 1 ].lower() in extensions or self.__warnExtension( d, _ ) ) ]		\
...
Scan current folder and sub folders and write a list "<folderName>.lst" to
each folder and each sub folder.
Usage: makeList.py

###########################################################################################################################
Scan specified folder "e:\music" and sub folders. Write an m3u list to any
...
the prefix "../" followed by the relative path.
Usage: makeList.py -d=e:\music --fmtOnlyEntries="" --fmtAllSubEntries="" -l=lists -p=../ -t=m3u

###########################################################################################################################
Just print all files to STDOUT
Usage: makeList.py -D -o=- -d=myDir
...
###########################################################################################################################
Copy all selected movies with partial parts "*\\HT0*" to the folder "c:\data\temp\list" using
an custom Python snippet.
makeList.py -a -o="" -t=movies -d="e:\gfx\Ausflüge" --filterSnippet="fnmatch.fnmatch( filePath, '*\\*HT0*')" --os="shutil.copyfile( filePath, 'c:\\data\\temp\\list\\' + os.path.basename( filePath ) )" -v
"""
...
gOutput.add_argument( "-t", "--type", dest = "outputType", help = "Type of output lists. Valid options are: fileList, pictures, movies, movies, m3u, m3uExt. Default: None", default = None )
...
gOutput.add_argument( "-s", "--filterSnippet", help = "Python snippet (type eval) to be customized while scanning to check, if an entry should be included or not. The following globals are defined: os, re, sys, glob, math, shutil, fnmatch, traceback. The following locals are defined: path, Default: Disabled", default = None )
...
if args.examples:
	...
	makeList	= MakeList( args.listsFolder, args.extensions if args.extensions is not None else None,
							args.ignore if args.ignore is not None else None, args.outputMode,
							args.fmtOnlyEntries, args.fmtAllSubEntries, args.fmtLists,
							args.filterSnippet, args.writeEmptyLists, args.outputType, args.dryMode,
							verbose or args.verboseVerbose, args.verboseVerbose )
	results					= []
